...
Y=B$Group.1
x1=B$x
ww=coord_cartesian(ylim = c(-1.3,-0.5))
...
p<-
  ...
  annotate("text", x = 2007.4, y = -0.60, label = "SPI",face="bold",size=35)+              # Text on plot
  annotate("text", x = 2005, y = -1.25, label = "y = -0.0409x + 81.137",face="bold",size=25)+
  ww+
  cleanup
...
z=p + geom_point(data=mx,aes(x=Y, y=x1),size=10)+ geom_line(data=mx,aes(x=Y,y=x1),linetype = "dotted", size = 6) 
z
ggsave(filename = "SPI_fittedALL1.jpeg",path="/Volumes/Sony_16GR/R2",z,dpi=800, width =24, height = 20 )
